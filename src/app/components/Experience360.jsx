...
const Experience360 = ({ images }) => {
  ...
  useEffect(() => {
    ...
    const loadImages = async () => {
      imageRefs.current = await Promise.all(images.map(src => {
      ...
      drawImage(ctx, currentIndex);
    };

    loadImages();
  }, [images]);

  const drawImage = (ctx, index) => {
    ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height);
    ctx.drawImage(imageRefs.current[index], 0, 0, ctx.canvas.width, ctx.canvas.height);
  };
  ...
  const handleMouseMove = (event) => {
    if (isDragging) {
      ...
      const newIndex = Math.floor(((moveX) / canvasRef.current.width) * imageCount);
      setCurrentIndex(Math.max(0, Math.min(imageCount - 1, newIndex)));
      drawImage(canvasRef.current.getContext('2d'), newIndex);
    }
  };
